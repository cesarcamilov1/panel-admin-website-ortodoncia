...
describe('servicesApi location allowlist', () => {
  const locationId = '22222222-2222-2222-2222-222222222222'
  const providerUserId = '33333333-3333-3333-3333-333333333333'

  it('lists the services enabled at a location', async () => {
    const { http, spies } = stubHttp({ get: vi.fn().mockResolvedValue({ items: [dto] }) })
    const result = await createServicesApi(http).listLocationServices({ locationId })

    expect(spies.get).toHaveBeenCalledWith(`/api/v1/schedules/locations/${locationId}/services`)
    expect(result[0].code).toBe('LIMP-01')
  })

  it('narrows the location listing by provider when one is given', async () => {
    const { http, spies } = stubHttp({ get: vi.fn().mockResolvedValue({ items: [] }) })
    await createServicesApi(http).listLocationServices({ locationId, providerUserId })

    expect(spies.get).toHaveBeenCalledWith(
      `/api/v1/schedules/locations/${locationId}/services?provider_user_id=${providerUserId}`,
    )
  })

  it('replaces the allowlist atomically and resolves with nothing on 204', async () => {
    const { http, spies } = stubHttp({ put: vi.fn().mockResolvedValue(undefined) })

    await expect(
      createServicesApi(http).replaceLocationServices({
        locationId,
        providerUserId,
        serviceIds: [dto.id, dto.id],
      }),
    ).resolves.toBeUndefined()

    expect(spies.put).toHaveBeenCalledWith(
      `/api/v1/schedules/locations/${locationId}/services`,
      { provider_user_id: providerUserId, service_ids: [dto.id] },
    )
  })

  it('rejects an allowlist longer than the 200 the contract allows', async () => {
    const { http, spies } = stubHttp({ put: vi.fn() })
    const serviceIds = Array.from({ length: 201 }, (_, index) => `id-${index}`)

    await expect(
      createServicesApi(http).replaceLocationServices({ locationId, providerUserId, serviceIds }),
    ).rejects.toBeInstanceOf(ApiError)
    expect(spies.put).not.toHaveBeenCalled()
  })

  it('lists the public services of a location without a session', async () => {
    const { http, spies } = stubHttp({ get: vi.fn().mockResolvedValue({ items: [dto] }) })
    await createServicesApi(http).listPublicLocationServices({ locationId, providerUserId })

    expect(spies.get).toHaveBeenCalledWith(
      `/api/v1/public/locations/${locationId}/services?provider_user_id=${providerUserId}`,
    )
  })
})
